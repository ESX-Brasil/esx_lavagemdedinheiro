	...
	window.addEventListener('message', function(event){		
		...
		if(data.showMenu){
			...
			for(let i=0; i<data.player.accounts.length; i++)
				if(data.player.accounts[i].name == 'black_money')
...
document.onkeyup = function (data) {
	if (data.which == 27) {
		$.post('http://esx_lavagemdedinheiro/escape', '{}');
	}
};
...
$('#deposit_btn').click(() => {
	$.post('http://esx_lavagemdedinheiro/deposit', JSON.stringify({
		amount: $('#deposit_amount').val()
	}));
})

$('#withdraw_btn').click(() => {
	$.post('http://esx_lavagemdedinheiro/withdraw', JSON.stringify({
